...
export const CopyToClipboard: React.FC<React.PropsWithChildren<{
  ...
  const pasteDataToClipboard = (event: any, clipboardData: any, data: string) => {
  ...
  useEffect(() => {
    const onCopy = async (event:any) => {
      ...
      if (event.clipboardData || event.originalEvent) {
        ...
        if (typeof copyData === 'string') { pasteDataToClipboard(event, clipboardData, copyData as string); }
      }
    };
